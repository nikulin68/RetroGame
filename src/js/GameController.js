...
export default class GameController {
  ...
  onCellClick(index) {
    ...
    if (this.personAction) {
      if (this.isPlayerPerson) {
        ...
        this.availablePoints = getAvailablePositions(
  ...
  async attackAction(ally, enemy) {
    const damage = Math.floor(Math.max(
      ally.character.attack - enemy.character.defence,
  ...
  computerAction() {
    const action = () => {
      const ally = this.gameState.computerTeamPositioned[getRandomInteger(
        0,
